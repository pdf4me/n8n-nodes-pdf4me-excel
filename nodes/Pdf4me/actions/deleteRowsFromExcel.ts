...
export const description: INodeProperties[] = [
	// === INPUT FILE SETTINGS ===
	{
		...
		displayOptions: {
			show: {
				operation: [ActionConstants.DeleteRowsFromExcel],
			},
		},
		options: [
			{
	...
	// === DELETE SETTINGS ===
	{
		...
		type: 'string',
		default: '',
		description: 'Name of the worksheet to delete rows from (leave empty to apply to all worksheets)',
		placeholder: 'Sheet1',
		displayOptions: {
			show: {
				operation: [ActionConstants.DeleteRowsFromExcel],
	...
	{
		displayName: 'Row Ranges',
		name: 'rowRanges',
		type: 'string',
		required: true,
		default: '',
		description: 'Comma-separated row ranges to delete. Use single numbers or ranges (e.g., "1-5,10,15-20" deletes rows 1-5, row 10, and rows 15-20)',
		placeholder: '10-20',
		displayOptions: {
			show: {
				operation: [ActionConstants.DeleteRowsFromExcel],
	...
	// === OUTPUT SETTINGS ===
	{
		...
		name: 'outputFileName',
		type: 'string',
		default: 'excel_rows_deleted.xlsx',
		description: 'Name for the processed Excel file (will have rows deleted)',
...
/**
 * Delete rows from Excel files using PDF4Me API
 * Process: Read Excel file → Encode to base64 → Send API request → Poll for completion → Save Excel file
 * Deletes specified row ranges from Excel worksheets
 */
export async function execute(this: IExecuteFunctions, index: number) {
	try {
		...
		const rowRanges = this.getNodeParameter('rowRanges', index) as string;
			...
			return [
				{
					json: {
						...
						rowRanges,
						message: worksheetName
							? `Successfully deleted rows ${rowRanges} from worksheet '${worksheetName}'`
							: `Successfully deleted rows ${rowRanges} from all worksheets`,
					},
					binary: {
