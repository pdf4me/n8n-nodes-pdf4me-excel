...
export async function execute(this: IExecuteFunctions, index: number) {
	try {
		...
		if (delimiterOption === 'custom') {
			if (!customDelimiter) {
				throw new NodeOperationError(this.getNode(), 'Custom delimiter is required when "Custom" is selected');
			}
		...
		if (!docContent || docContent.trim() === '') {
			throw new NodeOperationError(this.getNode(), 'CSV content is required');
		}
		...
		throw new NodeOperationError(this.getNode(), `Parse CSV to Excel failed: ${errorMessage}`);
	}
}
