	...
	try {
		...
		if (extractBy === 'name') {
			if (!worksheetNames || worksheetNames.trim() === '') {
				throw new Error('Worksheet names are required when extracting by name');
			}
		} else if (extractBy === 'index') {
			if (!worksheetIndexes || worksheetIndexes.trim() === '') {
				throw new Error('Worksheet indexes are required when extracting by index');
			}
		}
		...
		throw new Error(`Extract worksheet from Excel failed: ${errorMessage}`);
	}
}
