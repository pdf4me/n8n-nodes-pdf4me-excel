...
export const description: INodeProperties[] = [
	// === INPUT FILE SETTINGS ===
	{
		...
		displayOptions: {
			show: {
				operation: [ActionConstants.DeleteWorksheetFromExcel],
			},
		},
		options: [
			{
	...
	// === DELETE SETTINGS ===
	{
		...
		type: 'options',
		default: 'name',
		description: 'Choose whether to delete worksheets by name or by index',
		displayOptions: {
			show: {
				operation: [ActionConstants.DeleteWorksheetFromExcel],
			},
		...
		options: [
			{
				...
				value: 'name',
				description: 'Delete worksheets by their names',
			},
			{
				name: 'Worksheet Indexes',
				value: 'index',
				description: 'Delete worksheets by their index positions',
			},
		],
	},
	{
		...
		type: 'string',
		default: '',
		description: 'Comma-separated list of worksheet names to delete (e.g., "Sheet1, Sheet2, Data")',
	...
	},
	{
		...
		type: 'string',
		default: '',
		description: 'Comma-separated list of worksheet indexes to delete (1-based, e.g., "1, 2, 3")',
		placeholder: '1, 2, 3',
		displayOptions: {
			show: {
				operation: [ActionConstants.DeleteWorksheetFromExcel],
	...
	// === OUTPUT SETTINGS ===
	{
		...
		name: 'outputFileName',
		type: 'string',
		default: 'excel_worksheet_deleted.xlsx',
		description: 'Name for the processed Excel file (will have worksheet(s) deleted)',
...
/**
 * Delete worksheets from Excel files using PDF4Me API
 * Process: Read Excel file → Encode to base64 → Send API request → Poll for completion → Save Excel file
 * Deletes specified worksheets by name or index from Excel files
 */
export async function execute(this: IExecuteFunctions, index: number) {
	try {
		...
		const deleteBy = this.getNodeParameter('deleteBy', index, 'name') as string;
		...
		if (deleteBy === 'name') {
			if (!worksheetNames || worksheetNames.trim() === '') {
				throw new NodeOperationError(this.getNode(), 'Worksheet names are required when deleting by name');
			}
		} else if (deleteBy === 'index') {
			if (!worksheetIndexes || worksheetIndexes.trim() === '') {
				throw new NodeOperationError(this.getNode(), 'Worksheet indexes are required when deleting by index');
			}
		}

		// Determine which field to send based on deleteBy
		const finalWorksheetNames = deleteBy === 'name' ? worksheetNames : '';
		const finalWorksheetIndexes = deleteBy === 'index' ? worksheetIndexes : '';
		...
		const responseData = await pdf4meAsyncRequest.call(
			this,
			'/office/ApiV2Excel/ExcelDeleteWorksheet',
			body,
		);
		...
		if (responseData) {
			...
			let deletionInfo: string;
			if (deleteBy === 'name') {
				deletionInfo = `worksheet(s) '${worksheetNames}' by name`;
			} else {
				deletionInfo = `worksheet(s) at index(es) '${worksheetIndexes}'`;
			}
			...
			return [
				{
					json: {
						...
						originalFileName,
						deleteBy,
						worksheetNames: deleteBy === 'name' ? worksheetNames : undefined,
						worksheetIndexes: deleteBy === 'index' ? worksheetIndexes : undefined,
						message: `Successfully deleted ${deletionInfo}`,
					},
					binary: {
		...
		throw new NodeOperationError(this.getNode(), `Delete worksheet from Excel failed: ${errorMessage}`);
	}
}
